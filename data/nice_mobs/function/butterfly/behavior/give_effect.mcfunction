...
$execute unless data entity @s data.nice_mobs{effect:"minecraft:glowing"} run return run effect give @a[gamemode=survival,distance=..3] $(effect) 60 0 false
execute if data entity @s data.nice_mobs{effect:"minecraft:glowing"} run return run effect give @e[type=!item,distance=..16] minecraft:glowing 60 0 false