...
execute as @n[type=minecraft:interaction,tag=nice_mobs.jellyfish.crafted.interaction,distance=..16] if data entity @s interaction at @s unless block ~ ~ ~ #eden:jellyfish_can_survive run return fail

execute store result score $jellyfish_value eden.technical run random value 50..100
execute as @e[type=minecraft:interaction,tag=nice_mobs.jellyfish.crafted.interaction,distance=..16] if data entity @s interaction run scoreboard players operation @s nice_mobs.item.time_since_placed += $jellyfish_value eden.technical
